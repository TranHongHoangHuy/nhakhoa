...
const allDoctors = async (req, res) => {
  try {
    const [doctors] = await req.app.locals.db.execute(
      "SELECT id, name, speciality, degree, experience, about, available, address, image FROM doctors"
    );
  ...
  try {
    ...
    if (existingSlot.length > 0) {
      return res.json({
        success: false,
        message:
          "Slot already exists for this doctor on the selected date and time",
      });
    }
