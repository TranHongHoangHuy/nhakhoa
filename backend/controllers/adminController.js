...
const appointmentsAdmin = async (req, res) => {
  try {
    const [appointments] = await req.app.locals.db.execute(
      "SELECT a.*,b.name patname,c.slot_date,c.slot_time,d.name docname,e.title FROM appointments a left join users b on a.userId = b.id left join slots c on a.slotId = c.id left join doctors d on c.doctor_id = d.id left join services e on a.serviceId = e.id "
    );
...
const appointmentCancel = async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      )}</strong> đã bị đội ngũ hành chính của chúng tôi hủy bỏ.
                    </p>
                    <p style="font-size: 16px; line-height: 1.5; color: #555;">
...
const appointmentConfirm = async (req, res) => {
  try {
    ...
    await req.app.locals.db.execute(
      "UPDATE appointments SET isConfirm = 1 WHERE id = ?",
      [appointmentId]
    );
    await req.app.locals.db.execute(
    ...
    const mailOptions = {
      ...
      html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 5px; background-color: #f9f9f9;">
                    <h2 style="color: #333;">Xin chào ${users[0].name},</h2>
                    <p style="font-size: 16px; line-height: 1.5; color: #555;">
                        Chúng tôi vui mừng thông báo rằng cuộc hẹn của bạn với Bác sĩ <strong>${
                          doctors[0].name
                        }</strong> vào ngày <strong>${date.toLocaleDateString(
        "vi-VN",
        { timeZone: "Asia/Ho_Chi_Minh" }
      )}</strong> đã được xác nhận.
                    </p>
                    <p style="font-size: 16px; line-height: 1.5; color: #555;">
...
const addDoctor = async (req, res) => {
  try {
    ...
    if (password.length < 8) {
      return res.json({
        success: false,
        message: "Vui lòng nhập mật khẩu mạnh hơn (ít nhất 8 ký tự)",
      });
    }
    ...
    if (!allowedFileTypes.includes(imageFile.mimetype)) {
      return res.json({
        success: false,
        message: "Vui lòng tải lên tệp hình ảnh hợp lệ",
      });
    }
...
const editDoctor = async (req, res) => {
  try {
    ...
    const updateDoctorQuery = `
            ...
            } speciality = ?, degree = ?, experience = ?, about = ?, address = ?
            WHERE id = ?
        `;
    ...
    await req.app.locals.db.execute("DELETE FROM doc_ser WHERE doctor_id = ?", [
      doctorId,
    ]);
...
const editService = async (req, res) => {
  try {
    ...
    const updateParams = [title, sortdes, describe, fees];
...
const editNews = async (req, res) => {
  try {
    ...
    const updateParams = [title, sortdes, describe];
...
const addSlot = async (req, res) => {
  try {
    ...
    if (!validDate || !validTime) {
      return res
        .status(400)
        .json({ success: false, message: "Định dạng ngày giờ không hợp lệ" });
    }
...
const updateSlot = async (req, res) => {
  try {
    ...
    if (existingSlot.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "Không tìm thấy slot với ID này" });
    }
