...
const registerUser = async (req, res) => {
  try {
    ...
    if (password.length < 8) {
      return res.json({
        success: false,
        message: "Vui lòng nhập mật khẩu hơn 8 ký tự",
      });
    }
...
const getFeedbacksByDoctor = async (req, res) => {
  try {
    ...
    if (feedbacks.length === 0) {
      return res.json({
        success: false,
        message: "Không tìm thấy feedback cho bác sĩ này",
      });
    }
...
const bookAppointment = async (req, res) => {
  try {
    const { userId, docId, slotDate, slotTime, serviceId } = req.body;
    const dateSlot = new Date(slotDate);
    const formattedDate = `${dateSlot.getFullYear()}-${(dateSlot.getMonth() + 1)
      .toString()
      .padStart(2, "0")}-${dateSlot.getDate().toString().padStart(2, "0")}`;
    ...
    if (doctors.length === 0) {
      await db.query("ROLLBACK"); // Rollback nếu không có bác sĩ
    ...
    if (users.length === 0) {
      await db.query("ROLLBACK"); // Rollback nếu không tìm thấy người dùng
      return res.json({ success: false, message: "Người dùng không tồn tại" });
    }

    // Kiểm tra slot
    const [slots] = await db.execute(
      "SELECT * FROM slots WHERE slot_date = ? AND slot_time = ? AND doctor_id = ? AND is_booked = 0 FOR UPDATE",
      [formattedDate, slotTime, docId]
    );

    if (slots.length === 0) {
      await db.query("ROLLBACK"); // Rollback nếu slot không khả dụng
      return res.json({ success: false, message: "Thời gian đã có người đặt" });
    }
    ...
    if (services.length === 0) {
      await db.query("ROLLBACK"); // Rollback nếu không tìm thấy dịch vụ
      return res.json({ success: false, message: "Dịch vụ không tồn tại" });
    }
    ...
    await db.execute(
      "INSERT INTO appointments (userId, slotId, amount, date, serviceId) VALUES (?, ?, ?, ?, ?)",
      [userId, slot.id, services[0].price, dateNow, serviceId]
    );

    await db.execute("UPDATE slots SET is_booked = 1 WHERE id = ?", [slot.id]);
    ...
    res.json({
      success: true,
      message: "Đặt lịch hẹn thành công và email đã được gửi",
    });
  } catch (error) {
    try {
      await req.app.locals.db.query("ROLLBACK"); // Rollback nếu có lỗi
    } catch (rollbackError) {
...
const editAppointment = async (req, res) => {
  try {
    ...
    if (
      slot[0].id === appointment[0].slotId &&
      appointment[0].serviceId === serviceId
    ) {
    ...
    const [users] = await req.app.locals.db.execute(
      "SELECT * FROM users WHERE id = ?",
      [appointment[0].userId]
    );

    if (slot.length === 0) {
      return res.json({
        success: false,
        message: "Khung giờ đã được đặt, vui lòng chọn thời gian khác",
      });
    }
    ...
    res.json({
      success: true,
      message: "Cập nhật lịch hẹn thành công và email đã được gửi",
    });
  } catch (error) {
...
const cancelAppointment = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Đã hủy lịch hẹn và gửi email thông báo",
    });
  } catch (error) {
    console.log(error);
    res.json({ success: false, message: `Lỗi: ${error.message}` });
...
const allSlotUser = async (req, res) => {
  try {
    const [slots] = await req.app.locals.db.execute(
      "SELECT * FROM slots WHERE slot_date > CURRENT_DATE OR (slot_date = CURRENT_DATE AND slot_time > CURRENT_TIME) order by slot_date,slot_time"
    );
...
const inputUrl = async (req, res) => {
  ...
  const { appointmentId } = req.body;
  await req.app.locals.db.execute(
    "UPDATE appointments SET payment = 1 WHERE id = ?",
    [appointmentId]
  );
  res.sendStatus(200);
};
...
const ratingAppointment = async (req, res) => {
  ...
  try {
    ...
    await req.app.locals.db.execute(
      "UPDATE appointments SET isReview = 1 WHERE id = ?",
      [appointmentId]
    );
