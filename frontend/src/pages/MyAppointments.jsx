...
const MyAppointments = () => {
  ...
  const handleEditToggle = (index) => {
    ...
    const slotDoc = slots.filter(
      (doc) =>
        doc.doctor_id === parseInt(updatedAppointments[index].doctor_id, 10)
    );
    const obj = {
              ...
              >
                ...
                <div className="flex-1 text-sm text-[#5E5E5E]">
                  ...
                  {item.isEdit ? (
                    ...
                    >
                      {services
                        .filter((item1) =>
                          item.services
                            .split(", ")
                            .includes(item1.id.toString())
                        ) // Lọc dịch vụ theo bác sĩ
                        .map((item1) => {
                          const formattedVNDService = new Intl.NumberFormat(
                            "vi-VN",
                    ...
                    <p className="">
                      {item.service_name} - {formattedVND}
                    </p>
                  )}
                        ...
                        <div className="flex gap-3 items-center w-full overflow-x-scroll mt-2">
                          {slotArr.length !== 0 &&
                            [
                              ...new Map(
                                slotArr
                                  .filter((x) => x.is_booked === 0)
                                  .map((appt) => [
                                    new Date(appt.slot_date).toDateString(),
                                    appt,
                                  ])
                              ).values(),
                            ].map((appt, index) => {
                              ...
                              return (
                                <div
                                  key={index}
                                  onClick={() =>
                                    setSlotDateAsyn(appt.slot_date)
                                  }
                                  className={`text-center py-1 px-3 rounded-full cursor-pointer ${
                ...
                <div className="flex flex-col gap-2 justify-end text-sm text-center">
                  ...
                  {item.payment === 0 &&
                    ...
                    appointmentDateTime >= now && (
                      ...
                      >
                        {loadingAppointmentId === item.appointment_id
                          ? "Đang hủy...."
                          : "Hủy lịch hẹn"}
                      </button>
                    )}
                  ...
                  {item.isEdit && (
                    <button
                      ...
                      disabled={
                        loadingEditAppointmentId === item.appointment_id
                      }
                    >
                      {loadingEditAppointmentId === item.appointment_id
                        ? "Dang lưu...."
                  ...
                  {item.payment === 1 && !item.isCompleted && (
                    <button className="sm:min-w-48 py-2 border border-green-500 rounded text-green-500">
                      Đã thanh toán
                    </button>
                  )}
